		...
		<?php 
			...
			if (isset($_POST['delete'])) 
			{
				...
				if ($pass==$passwd)
				{
					...
					{// DELETE ASSET like laptop desktop etc. from admin panel.
					...
					}elseif (isset($_GET['Del_Rent']))
						{ //  Delete Rent Related data from rent Table id =2;
							$delrent=$_GET['Del_Rent']; // GET ID FROM URL
							$query = mysqli_query($conn,"DELETE FROM `rent` WHERE `id`='$delrent'");
							if (! $query) {$msg = "Unable To Delete Records";}else{ $msg = "Deleted Successfuly";}
						...
						$update=$_GET['upass']; // GET ID FROM URL
						$new=md5('123456');
						
							//UPDATE `users` SET `password`=$new WHERE `id`=$update
							$query = mysqli_query($conn,"UPDATE users SET password='$new' WHERE id='$update'");
							if (! $query) {$msg = "Unable To Delete Records";}else{ $msg = "User Passowrd Changed Successfull New Passowrd is 123456";}
					...
					header("refresh:7;" .$index);
			                      ...
			                      <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
			                       <strong>Failed!</strong> Invalid Password <a href="" class="alert-link"> Check Password and</a>, Try Again.
	                  			</div>';}
			}
		?>
						...
						<form action="" method="post">
							...
							<div class="row">
								...
								<div class="col-xs-7 text-right">
									<button type="submit" name="delete" class="btn btn-danger"><i class="fa  fa-trash-o" > </i>  Delete Everything</button>
								</div>
							</div>
						</form>
					</div>
