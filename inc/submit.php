...
if (isset($_POST['btn_sub'])) {
	...
	if (empty($user_name) || empty($user_email) || empty($req_desc) || empty($user_id)) {
		$msg= '<div class="alert alert-danger">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
					<strong> Field is Blank!</strong> All Fileld is Required.
				</div>';
				...
				$sql="INSERT INTO `itsupport`(`date`, `name`, `email`, `userid`, `sysid`, `probs`, `status`,`priority`)
				 VALUES ('$req_dt','$user_name','$user_email','$user_id','$serial','$req_desc','$req_status','$priority')";
				// echo "$sql";
				$query=mysqli_query($conn,$sql);
				if ($query) {
					$msg= '<div class="alert alert-success">
